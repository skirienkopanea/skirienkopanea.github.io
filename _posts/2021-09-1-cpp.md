...
* Both C and C++ language program are compiled into machine code, but are (not always) machine independent (a feature of high-level). Java is machine indendent because it's run on JVM
...
* `std::cout`:
  ...
  * The `cout` API abstracts the output to the terminal such that it's a "stream" and can be easily fed with multiple inputs
...
* The `new` keyword performs memory allocation
  * returns a pointer
