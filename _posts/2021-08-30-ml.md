...
- [Table of Contents](#table-of-contents)
  ...
  - [Machine Learning is soft classification](#machine-learning-is-soft-classification)
    - [Machine Learning Pipeline](#machine-learning-pipeline)
    - [Non-Bayesian Example: Polynomial Curve Fitting (Linear Model)](#non-bayesian-example-polynomial-curve-fitting-linear-model)
...
## Machine Learning is soft classification
* ML learning (algorithms whose performance improves with experience) regards "soft" classication tasks
  * Cases such as a handwritten digit recognition in which the aim is to assign each input vector (image) to one of a finite number of **discrete** categories (0-9 digit), are called **classification**
problems.
  * If the desired output consists of one or more **continuous** variables, (like predecting a house price) the tasks is called **regression**
  * All the other ML tasks are in some way also a discrete or continuous classification
* Soft classifiers estimate the label **conditional probabilities** based on **Baye's rule** (previous facts update the probability)
* In contrast, hard classifiers directly target on the classification decision boundary without producing the probability estimation (i.e. FRISS hardcoded "indicators" (underage driving = always fraud, no need for fancy algorithms, just "if else")).
* Soft classification is done using one or multiple ML techniques. This course covers:
...
### Non-Bayesian Example: Polynomial Curve Fitting (Linear Model)
* Regression problem used to introduce some terms
