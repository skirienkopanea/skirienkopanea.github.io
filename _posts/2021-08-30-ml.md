...
* In theory minimizing that loss function would require us to know the actual class distributions (and calculate their averages (expectect values)), which in linear models we don't know.
  ...
  * Then select the hypothesis version (i) function with the minimum loss
...
* It emerges from the fact that the classical loss function of % of wrong classess assigned does not fit well with the gradient descent algorithm
  ...
  * The logistic regression therefore has a different approach: find a function that approximates the posterior probabilities
    * The change in class posterior probability percentage (i.e. confidence of the classification of given object) does react to slight modifications of the discriminant function! (which fits well with the gradient descent algorithm)
* $$h(x)\approx p(y|x)$$
  * Where `h(x)` returns a continous value between [0,1]

#### Logistic function
![404]({{ site.url }}/images/ml/logistic_function.png)
* This works for random variable y being a discrete value 0 or 1, but recall that `p(y|x)` is a contiunous value, which we will estimate using \\(z=\sigma(h(x))\\) aka the logistic function (instead of using bayes the density will be bernoulli)
...
* The reason we are transforming `h(x)=p(y|x)` to a logistic function is that this will allow us to aggregate the posterior probabilities for all clases into a bernoulli distribution that is eventually transformed into the cost function.
